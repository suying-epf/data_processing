...
object ClimateService {
  ...
  def isClimateRelated(sentence: String): Boolean = {
    val climateKeywords = List("climate", "global warming", "greenhouse effect", "carbon dioxide")
...
/*def estimateCO2LevelsFor2050(list: List[CO2Record]): Double = {
  val (min, max) = getMinMax(list)
  val avgAnnualIncrease = (max - min) / (list.map(_.year).max - list.map(_.year).min)
  val yearsTo2050 = 2050 - list.map(_.year).max
  val estimatedIncrease = avgAnnualIncrease * yearsTo2050
  val lastRecordPpm = list.filter(_.year == list.map(_.year).max).headOption.map(_.ppm).getOrElse(0.0)
  lastRecordPpm + estimatedIncrease
}*/
